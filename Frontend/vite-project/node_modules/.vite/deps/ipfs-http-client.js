...
function encodeUint8Array(value, buf2, offset = 0) {
  switch (encodingLength6(value)) {
    case 8: {
      buf2[offset++] = value & 255 | MSB6;
      value /= 128;
    }
    case 7: {
      buf2[offset++] = value & 255 | MSB6;
...
function encodeUint8ArrayList(value, buf2, offset = 0) {
  switch (encodingLength6(value)) {
    case 8: {
      buf2.set(offset++, value & 255 | MSB6);
      value /= 128;
    }
    case 7: {
      buf2.set(offset++, value & 255 | MSB6);
...
function encode25(value, buf2, offset = 0) {
  if (buf2 == null) {
    buf2 = allocUnsafe2(encodingLength6(value));
  }
  if (buf2 instanceof Uint8Array) {
...
function decode33(buf2, offset = 0) {
  if (buf2 instanceof Uint8Array) {
...
function concat3(arrays, length7) {
  ...
  const output = allocUnsafe3(length7);
  let offset = 0;
  for (const arr of arrays) {
    output.set(arr, offset);
    offset += arr.length;
...
function decode34(string3, alphabet4, bitsPerChar, name10) {
  const codes3 = {};
  for (let i = 0; i < alphabet4.length; ++i) {
    codes3[alphabet4[i]] = i;
  }
...
function encode26(data, alphabet4, bitsPerChar) {
  const pad = alphabet4[alphabet4.length - 1] === "=";
  const mask = (1 << bitsPerChar) - 1;
  let out = "";
  let bits = 0;
...
function rfc46486({ name: name10, prefix, bitsPerChar, alphabet: alphabet4 }) {
  return from8({
    ...
    name: name10,
    encode(input) {
      return encode26(input, alphabet4, bitsPerChar);
    },
    decode(input) {
      return decode34(input, alphabet4, bitsPerChar, name10);
    }
  });
}
...
function decode38(multihash) {
  ...
  const [code10, sizeOffset] = decode37(bytes);
...
function digest3(input) {
  return create6(code9, encode29(input));
}
var identity6 = { code: code9, name: name9, encode: encode29, digest: digest3 };
...
var CID6 = class _CID {
  ...
  static asCID(input) {
    ...
    if (value instanceof _CID) {
      ...
      const { version, multihash, code: code10 } = value;
      const digest4 = decode38(multihash);
      return _CID.create(version, code10, digest4);
    } else {
      return null;
    }
  ...
  static inspectBytes(initialBytes) {
    let offset = 0;
    const next = () => {
      const [i, length7] = decode37(initialBytes.subarray(offset));
      offset += length7;
      return i;
    };
    let version = next();
...
function str2bytes2(str) {
  ...
  const size = Uint8Array.from(encode25(buf2.length));
  return concat3([size, buf2], size.length + buf2.length);
}
function bytes2str2(buf2) {
  const size = decode33(buf2);
  buf2 = buf2.slice(encodingLength6(size));
  if (buf2.length !== size) {
    throw new Error("inconsistent lengths");
  }
...
function bytes2mh2(buf2) {
  const size = decode33(buf2);
  const address = buf2.slice(encodingLength6(size));
  if (address.length !== size) {
    throw new Error("inconsistent lengths");
  }
...
function stringToMultiaddrParts(str) {
  ...
  for (let p = 0; p < parts.length; p++) {
    ...
    p++;
    if (p >= parts.length) {
      throw new ParseError2("invalid address: " + str);
    }
    if (proto.path === true) {
...
function bytesToMultiaddrParts(bytes) {
  ...
  while (i < bytes.length) {
    const code10 = decode33(bytes, i);
    const n = encodingLength6(code10);
    ...
    i += size + n;
    if (i > bytes.length) {
      throw new ParseError2("Invalid address Uint8Array: " + toString7(bytes, "base16"));
    }
    tuples.push([code10, addr]);
...
function tuplesToBytes2(tuples) {
  return concat3(tuples.map((tup) => {
    const proto = getProtocol2(tup[0]);
    let buf2 = Uint8Array.from(encode25(proto.code));
    if (tup.length > 1 && tup[1] != null) {
...
function sizeForAddr2(p, addr) {
  if (p.size > 0) {
    ...
    const size = decode33(addr instanceof Uint8Array ? addr : Uint8Array.from(addr));
    return size + encodingLength6(size);
  }
}
...
__publicField(ParseError2, "name", "ParseError");
...
var _Multiaddr = class _Multiaddr {
  ...
  toOptions() {
    ...
    for (const [code10, value] of this.stringTuples()) {
      ...
      if (code10 === tcp.code || code10 === udp.code) {
        transport = getProtocol2(code10).name === "tcp" ? "tcp" : "udp";
        port = parseInt(value ?? "");
      }
      if (code10 === ip4.code || code10 === ip6.code) {
        transport = getProtocol2(code10).name === "tcp" ? "tcp" : "udp";
  ...
  tuples() {
    return __privateGet(this, _tuples).map(([code10, value]) => {
      if (value == null) {
        return [code10];
      }
      return [code10, value];
  ...
  stringTuples() {
    return __privateGet(this, _stringTuples).map(([code10, value]) => {
      if (value == null) {
        return [code10];
      }
      return [code10, value];
